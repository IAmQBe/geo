...
class YandexMapsParser(BaseParser):
    ...
    def _looks_like_place(self, row: dict) -> bool:
        ...
        has_name = any(key in keys for key in ("name", "title", "displayname", "company_name", "orgname"))
        has_location = any(
            key in keys
            for key in (
                "address",
        ...
        has_business_hint = any(key in keys for key in ("rating", "reviews", "rubrics", "categories", "phones"))
        ...
        script = """
            () => {
                ...
                Array.from(nodes).slice(0, 300).forEach((node) => {
                    const nameEl = node.querySelector('a[class*="link-overlay"], h1, h2, [class*="title"]');
                    const addressEl = node.querySelector('[class*="address"], [class*="subtitle"]');
                    const ratingEl = node.querySelector('[class*="rating"]');
                    const hrefEl = node.querySelector('a[href*="/org/"], a[href*="yandex.ru/maps"]');
    ...
    def _to_place(self, candidate: dict, search_url: str) -> ParsedPlace | None:
        name = find_text(candidate, {"name", "title", "displayname", "company_name", "orgname"})
        ...
        source_id = find_text(candidate, {"id", "oid", "businessid", "orgid", "objectid", "uid"})
        if source_id:
